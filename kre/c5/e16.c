...
int dostrcmp(char *s, char *t);
int fcdostrcmp(char *s, char *t);
...
main(int argc, char *argv[])
{
	int c;
	int nlines;
	int numeric = 0, reversed = 0, foldcase = 0, dirord = 0;
	...
	/* todo: FP metaphors to express function composition
	 *	here in argparsing rather than with additional
	 *	program-specific names
	 */
	if(numeric)
		cmpfnptr = (int (*)(void *, void *)) numcmp;
	else if(foldcase && dirord)
		cmpfnptr = (int (*)(void *, void *)) fcdostrcmp;
	else if(foldcase)
		cmpfnptr = (int (*)(void *, void *)) fcstrcmp;
	else if(dirord)
		cmpfnptr = (int (*)(void *, void *)) dostrcmp;
	else
...
int
fncompstrcmp(char *s, char *t, void (*pre)(char *))
{
...
{
	return fncompstrcmp(s, t, lower);
}

int
dostrcmp(char *s, char *t)
{
	return fncompstrcmp(s, t, dirord);
}

int
fcdostrcmp(char *s, char *t)
{
	return fncompstrcmp(s, t, lowerdirord);
}
