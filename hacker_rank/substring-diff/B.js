...
var maxD = function (seq, S) {
  ...
  var i;
  var oneIdx, zeroIdx, prevSeq, prevSeqStartIdx;
  for (i = 1; i < seq.length; i += 1) {
    if (mData[i-1].sum + seq[i] <= S) {
      ...
      prevSeqStartIdx = i - 1 - mData[i-1].len + 1;
      prevSeq = seq.slice(prevSeqStartIdx);
      oneIdx = prevSeq.indexOf(1) + prevSeqStartIdx;
      mData[i] = {
        len: i - (oneIdx + 1) + 1,
        sum: mData[i-1].sum
      };
  ...
  var mD = mData.map(function (o) {return o.len;}).reduce(myMax);
  return mD;
};
